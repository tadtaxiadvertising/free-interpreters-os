...
const prisma = globalForPrisma._prismaApi ?? createPrismaClient();
...
export async function warmupPrisma() {
    try {
        ...
        console.error('🔴 PRISMA: Failed to connect at startup:', err instanceof Error ? err.message : err);
...
export async function disconnectPrisma() {
    if (globalForPrisma._isShuttingDown)
        return;
    ...
    try {
        ...
        console.error('⚠️ PRISMA: Error during disconnect:', err instanceof Error ? err.message : err);
    }
}
