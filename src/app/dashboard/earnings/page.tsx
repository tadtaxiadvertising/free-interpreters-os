  ...
  if (!interpreter) {
    return (
      <div className="p-12 glass rounded-3xl border border-white/5 text-center">
        ...
        <h3 className="text-xl font-bold text-white">Profile Not Linked</h3>
        <p className="text-gray-400 mt-2">Your user account is not yet linked to an interpreter profile. Please contact support.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header>
        ...
        <p className="text-gray-400 mt-2">Track your income, rates, and payment history.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass p-6 rounded-3xl border border-white/5">
          ...
          <div className="flex items-center gap-3 mt-2">
            <h3 className="text-xl font-bold text-white">{interpreter.metodoPago || 'Not Set'}</h3>
          </div>
          <div className="mt-4 text-xs text-gray-500 truncate">{interpreter.cuentaPago || 'Add your payment details in Settings'}</div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-white/5">
              {interpreter.payrollRecords.map((record) => (
