...
import { getRbacCalendarData } from "@/app/actions/rbac-data";
import {
  Flame,
  Trophy,
  TrendingUp,
...
export default async function InterpreterCalendarPage() {
  const commitment = await getRbacCalendarData();

  if (!commitment) {
    return (
      <div className="p-8 text-white">
        Perfil de intérprete no vinculado. Contacta al administrador.
      </div>
    );
  }

  const tariff = commitment.interpreter.tariffPerMinute
    ? Number(commitment.interpreter.tariffPerMinute)
    : 0;
  ...
  const formatMinutes = (mins: number) =>
    new Intl.NumberFormat("en-US").format(mins);
  const formatMoney = (amount: number) =>
    `RD$${new Intl.NumberFormat("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(amount)}`;
    ...
    <div className="p-8 max-w-5xl mx-auto text-white">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-end gap-2">
              <span className="text-5xl font-black text-white tracking-tight">
                {formatMinutes(commitment.totalMinutesWeek)}
              </span>
              <span className="text-slate-400 mb-2 font-medium">
                / {formatMinutes(commitment.targetMinutesToDate)} min esperados
              </span>
            </div>
            ...
            <div className="mt-5 h-3 bg-slate-800 rounded-full overflow-hidden border border-slate-700/50">
              <div
                className={`h-full rounded-full transition-all duration-1000 ease-out ${
                  isOverachieving
                    ? "bg-gradient-to-r from-emerald-400 to-emerald-300 shadow-[0_0_15px_rgba(52,211,153,0.8)]"
                    : commitment.healthScore >= 100
                      ? "bg-emerald-500"
                      : commitment.healthScore >= 80
                        ? "bg-amber-500"
                        : "bg-rose-500"
                }`}
                style={{
                  width: `${Math.min(100, commitment.healthScore)}%`,
                }}
              />
            </div>
        ...
        {commitment.deficit > 0 ? (
          <div className="bg-gradient-to-br from-slate-900 to-rose-950/20 border border-rose-500/20 rounded-2xl p-6 relative overflow-hidden group hover:border-rose-500/40 transition-all duration-300">
            ...
            <p className="text-sm text-slate-300 mb-5 leading-relaxed">
              El fin de semana es tu oportunidad. Recupera tus{" "}
              <span className="font-bold text-rose-400">
                {formatMinutes(commitment.deficit)} minutos
              </span>{" "}
              pendientes y asegura{" "}
              <span className="font-bold text-emerald-400">
                {formatMoney(recoveryMoney)} extra
              </span>{" "}
              en tu próximo pago.
            </p>
              ...
              <div className="space-y-3 relative z-10">
                <div className="bg-black/40 backdrop-blur-sm p-4 rounded-xl flex justify-between items-center border border-white/5">
                  ...
                  <span className="font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-lg">
                    +{formatMinutes(commitment.recoverySuggestions.saturdayMinutes)} min
                  </span>
                </div>
                <div className="bg-black/40 backdrop-blur-sm p-4 rounded-xl flex justify-between items-center border border-white/5">
                  <span className="text-slate-200 font-medium">Domingo</span>
                  <span className="font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-lg">
                    +{formatMinutes(commitment.recoverySuggestions.sundayMinutes)} min
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="bg-gradient-to-br from-emerald-900/40 to-slate-900 border border-emerald-500/30 rounded-2xl p-6 flex flex-col justify-center items-center text-center group hover:border-emerald-500/60 transition-all duration-300">
            ...
            <p className="text-slate-300 max-w-[250px]">
              Estás al día con tus metas. Todo lo que hagas ahora es{" "}
      ...
      <div className="flex items-center gap-3 mb-6 mt-10">
        ...
        <span className="text-xs font-medium text-slate-500 bg-slate-800 px-2 py-1 rounded-md">
          Meta: {formatMinutes(commitment.dailyGoalMinutes)} min/día
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
        {commitment.dailyStats.map((stat) => {
          const isOverproducing =
            stat.minutes >= commitment.dailyGoalMinutes * 1.1;
          ...
          return (
            ...
            >
              ...
              <p
                className={`text-xs font-medium mb-3 ${isMet ? "text-slate-300" : "text-slate-500"}`}
              >
                {new Date(stat.date + "T12:00:00")
              ...
              <div className="flex items-baseline gap-1">
                ...
                <span
                  className={`text-sm font-medium ${isMet ? "text-emerald-500/70" : "text-slate-600"}`}
                >
                  min
                </span>
              </div>
